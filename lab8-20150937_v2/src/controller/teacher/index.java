...
public class index extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if(user == null){
			...
			Query query = pm.newQuery(Teacher.class);
			//query.setOrdering("nombre descending");
			List<Teacher> array = (List<Teacher>)query.execute("select from Profesor");

			req.setAttribute("array", array);
			try {
				req.getRequestDispatcher("/WEB-INF/Views/Teacher/index.jsp").forward(req, resp);
				query.closeAll();
